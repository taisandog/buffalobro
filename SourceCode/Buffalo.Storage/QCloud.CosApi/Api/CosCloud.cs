...
namespace QCloud.CosApi.Api
{
	public partial class CosCloud
	{
        ...
        //const string COSAPI_CGI_URL = "http://mystorage-1251039330.cosgz.myqcloud.com/";
		//文件大于8M时采用分片上传,小于等于8M时采用单文件上传
		
		//用户计算用户签名超时时间
		const int SIGN_EXPIRED_TIME = 180;
		//HTTP请求超时时间
		public const int Default_HTTP_TIMEOUT_TIME = 60*1000;
		private int _appId;
		...
		public string GetFileStat(string bucketName, string remotePath)
		{
			...
			var header = new Dictionary<string, string>();
			header.Add("Authorization", sign);
			return _httpRequest.SendRequest(url, ref data, HttpMethod.Get, ref header, _timeOut);
		}
		                          ...
		                          string bizAttribute="", int insertOnly=1)
		{
			...
			var header = new Dictionary<string, string>();
			header.Add("Authorization", sign);
			return _httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, _timeOut, localPath);
		}
        ...
        public string SliceUploadData(string bucketName, string remotePath, string localPath, string fileSha, string session, long offset,int sliceSise,string sign)
        {
            ...
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return _httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, _timeOut, localPath, offset,sliceSise);
        }
