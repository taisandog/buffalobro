    ...
    internal class ResumableUploadManager
    {
                                   ...
                                   EventHandler<StreamTransferProgressArgs> uploadProgressCallback)
        {
            ...
            try
            {
                ...
                WaitAllEvents(taskFinishEvents);
                allTaskDone = true;
            }
            finally
            {
        ...
        /// 等待结束
        /// </summary>
        /// <param name="taskFinishedEvents"></param>
        private static void WaitAllEvents(ManualResetEvent[] taskFinishedEvents)
        {
            if (Thread.CurrentThread.GetApartmentState() == ApartmentState.STA)
            {
                foreach (ManualResetEvent evnt in taskFinishedEvents)
                {
                    if (evnt == null)
                    {
                        continue;
            ...
            WaitHandle.WaitAll(taskFinishedEvents);
